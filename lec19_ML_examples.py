...
class NeuralNetwork:
    ...
    def copy(self):
        nn = NeuralNetwork([1],self.learning_rate)
        nn.W = copy.deepcopy(self.W)
        nn.g = copy.deepcopy(self.g)
        nn.b = copy.deepcopy(self.b)
        nn.g_prime = copy.deepcopy(self.g_prime)
        return nn
...
def hidden_func(x):
    return  3*math.cos(1+0.5*x)*(math.sin(x)-0.25)**2+np.random.normal(loc=0)*2
...
nn1 = NeuralNetwork([1, 1, 1, 1], 0.05)
#nn2 = nn1.copy()
#nn2.learning_rate = 0.01
mse_list1 = []
mse_list2 = []
num_steps = 200
for i in range(num_steps):
    nn1.gradient_descent(training_data_points)
    mse = nn1.mean_squared_error(testing_data_points)
    mse_list1.append(mse)
    #nn2.gradient_descent(training_data_points)
    mse = nn1.mean_squared_error(training_data_points)
...
plt.plot([i for i in range(1, num_steps+1)], mse_list1, linewidth=2.5, label="Testing MSE")
plt.plot([i for i in range(1, num_steps+1)], mse_list2, linewidth=2.5, label="Training MSE")
...
plt.grid()
plt.legend()
plt.savefig("nn_mse.png", dpi=500)

plt.clf()
plt.plot([i for i in range(50, num_steps+1)], mse_list1[49:], linewidth=2.5, label="Testing MSE")
plt.plot([i for i in range(50, num_steps+1)], mse_list2[49:], linewidth=2.5, label="Training MSE")
...
plt.grid()
plt.legend()
plt.savefig("nn_mse2.png", dpi=500)